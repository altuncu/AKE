...
class KeywordExtractor(object):
    ...
    def __init__(self, lan="en", n=3, dedupLim=0.9, dedupFunc='seqm', windowsSize=1, top=20, features=None, stopwords=None, thes=None, wiki=None, pos=False):
        self.lan = lan
    ...
    def extract_keywords(self, text):
        try:
            ...
            noun_phrases = heuristics.extract_noun_phrases(text)

            if self.dedupLim >= 1.:
                return heuristics.apply_heuristics(text, [ (cand.H, cand.unique_kw) for cand in todedup], self.thes, self.wiki, self.pos, self.lan)[:self.top]
            ...
            for cand in todedup:
                ...
                for (h, candResult) in resultSet:
                    ...
                    if dist > self.dedupLim:
                        toadd = False
                        break
                if self.pos == True and cand.unique_kw not in noun_phrases:
                    toadd = False    
                if toadd:
            ...
            return heuristics.apply_heuristics(text, [ (cand.kw,h) for (h,cand) in resultSet], self.thes, self.wiki, self.pos, self.lan)
